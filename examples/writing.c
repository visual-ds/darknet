...
void test_writing(char *cfgfile, char *weightfile, char *filename, char *outfile)
{
    network *net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(net, weightfile);
    ...
    while(1){
        ...
        if (outfile) {
            printf("Save image as %s.jpg (shape: %d %d)\n", outfile, pred.w, pred.h);
...
void run_writing(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "test")) test_writing(cfg, weights, filename, outfile);
}
